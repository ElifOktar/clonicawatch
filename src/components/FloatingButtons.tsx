...
export function FloatingButtons() {
  return (
    <>
      <div className="fixed bottom-20 right-3 md:bottom-6 md:right-6 z-50 flex flex-col gap-3">
        {/* WhatsApp Button */}
        <a
        ...
        >
          {/* Pulse ring */}
          <span className="absolute inset-0 rounded-full bg-[#0088cc]/40 animate-ping-slow-delayed" />
          <svg className="w-7 h-7 md:w-8 md:h-8 text-white relative z-10" fill="currentColor" viewBox="0 0 24 24">
      ...
      <style jsx>{`
        ...
        .animate-float-tg {
          animation: float-tg 3s ease-in-out infinite 0.5s;
        }
        .animate-ping-slow {
          animation: ping-slow 2.5s cubic-bezier(0, 0, 0.2, 1) infinite;
        }
        .animate-ping-slow-delayed {
          animation: ping-slow 2.5s cubic-bezier(0, 0, 0.2, 1) infinite 1.2s;
        }
      `}</style>
    </>
  );
