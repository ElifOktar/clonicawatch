...
export function BrandCircles() {
  return (
    <section className="bg-bg-elev border-y border-line py-6 md:py-8">
      <div className="container">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          >
            <div className="flex gap-4 md:gap-6 px-1 py-2">
              {BRANDS.map((brand) => (
                <Link
                  key={brand.slug}
                ...
                >
                  <div className={`w-[60px] h-[60px] md:w-20 md:h-20 rounded-full ${brand.logo ? "bg-white" : "bg-pink-500/10"} border-2 ${brand.slug === "ladies" ? "border-pink-400/30 group-hover:border-pink-400" : "border-gold/20 group-hover:border-gold"} flex items-center justify-center group-hover:shadow-[0_0_20px_rgba(201,168,76,0.15)] transition-all duration-300 overflow-hidden p-2.5 md:p-3`}>
                    {brand.logo ? (
                      <>
                      ...
                      <svg className="w-7 h-7 md:w-8 md:h-8 text-pink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                      </svg>
                    )}
                  </div>
                  <p className="text-[10px] md:text-[11px] text-ink-muted text-center w-16 md:w-20 truncate group-hover:text-gold transition-colors">
                    {brand.name}
                  </p>
                </Link>
              ))}
